...
export default function useGame() {
  ...
  const handleSquareClick = (clickedRow, clickedColumn) => {
    // 點擊已經有棋子的位置或是已經有勝負
    if (history[currentStep].board[clickedRow][clickedColumn] || winningLine) /* eslint-disable-line curly */
      return;
...
const countConsecutivePieces = (
  ...
  let winningPieces = []; /* eslint-disable-line prefer-const */
  // winningPieces 一直是指向同一個記憶體位置沒有變，但對我來說值還是因為 .push() 變不一樣了，私心想用 let，雖然不知道這樣好不好 囧
  let tempRow = row;
